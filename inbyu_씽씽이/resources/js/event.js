$(function() {
    ...
    $('.all-check').on('click' , function () {
        if (this.checked) {
            ...
            $('.btn-agree.btn-primary').removeClass('disabled');
        }else {
            $(':checkbox').prop('checked', false);
            $('.btn-agree.btn-primary').addClass('disabled');
        }
    });
    ...
    $body.on('click', '.e-layer', function (e) {
        ...
        $window.scrollTop(0);
        $(targetLayer).css('min-height', winHeight).fadeIn();
        // console.log($(targetLayer).height());
    ...
    $body.on('click', '.layer .e-close, .layer .dimmed', function (e) {
        ...
        findParent.removeAttr('style').fadeOut();
        e.preventDefault();
    });
