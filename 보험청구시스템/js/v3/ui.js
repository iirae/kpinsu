...
$(window).on('load', function(){
    //$("html,body").stop().animate({"scrollTop":"0"}, .05);
	$("#visual .main-visual .slick-arrow>span").addClass("on");
});
...
var project = {
	...
	newMain : function(){
		...
		$.each(arr, function(){
			...
			if($val == $gP){
				...
				$groupSlider.find(".obj").eq(this).prepend("<div class='sticker'>모집성공</div>");
			}
			if($val.length <= 2){
		...
		controller.addTween( ".service_Slider", TweenMax.to( ".service_Slider", 1, {css:{"margin-top":"60px", opacity:1}, ease:Quad.easeOut}));
	...
	main : function(){
		...
		target.find("div").each(function(index){
			...
			for(var e = 1; e<= l; e++){
				TweenMax.to("#title_1", TweenMax.to( $("#title_1"), 1, {css:{"margin-top":"-200px", opacity:1}, ease:Quad.OutCirc}));
				TweenMax.to(".about-Box #obj_"+e, TweenMax.to( $(".about-Box #obj_"+e), e * .6, {css:{top:"0", opacity:1}, ease:Quad.OutCirc}));
			};
		});
		/*
		var target = $(".about-Box");
