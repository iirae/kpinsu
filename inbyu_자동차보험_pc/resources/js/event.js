$(function() {
    ...
    $document.on('click', '.js-modal', function(e){
        e.preventDefault();
        var $modal = $(this.hash),
            _margin = ($modal.find('.md-modal-foot').length) ? 120 : 60;
    ...
    $document.on('click', '.js-modal-close', function(e){
        ...
        if ($(this).closest('div').hasClass('md-modal-head')) {
            if ($modal.is('#modalDistance')) {
                $('#add1').prop('checked', false);
            } else if ($modal.is('#modalChildren')) {
                $('#add3').prop('checked', false);
            }
        }
    });
    ...
    if ($('#container').height() < $window.height()) {
        $html.addClass('is-view-mini');
    } else {
        $html.removeClass('is-view-mini');
    }
